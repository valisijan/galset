...
import { chats, users, ads, messages } from '@/lib/db/schema';
import { eq, and, isNull, or, desc, lt, count, inArray } from 'drizzle-orm';
...
router.post('/send', async (req: Request, res: Response) => {
  try {
    ...
    if (!conversationId) {
      ...
      const adCondition = adId ? eq(chats.adId, Number(adId)) : isNull(chats.adId);
      const conversation = await db.query.chats.findFirst({
        where: and(eq(chats.user1Id, user1Id), eq(chats.user2Id, user2Id), adCondition),
      });
    ...
    await db.update(chats).set({ updatedAt: now }).where(eq(chats.id, conversationId as string));

    const [senderUser, receiverUser, adData, conversationData] = await Promise.all([
      db.query.users.findFirst({ where: eq(users.id, Number(senderId)) }),
      db.query.users.findFirst({ where: eq(users.id, Number(receiverId)) }),
      adId ? db.query.ads.findFirst({ where: eq(ads.id, Number(adId)) }) : Promise.resolve(null),
      db.query.chats.findFirst({ where: eq(chats.id, conversationId as string) }),
    ]);
    ...
    if (supabaseUrl && supabaseKey) {
      fetch(`${supabaseUrl}/realtime/v1/api/broadcast`, {
        ...
        .then((r) => { if (!r.ok) r.text().then(t => console.error('Broadcast error response:', r.status, t)); })
        .catch((err: any) => console.error('Broadcast REST error:', err));
    }

    // Push notifikacija
    _sendPushForMessages(receiverId, senderId, senderName, conversationId as string)
...
async function _sendPushForMessages(receiverId: any, senderId: any, senderName: string, conversationId: string) {
  try {
    const unreadMessages = await db.select().from(messages).where(
      and(
        eq(messages.receiverId, Number(receiverId)),
        eq(messages.senderId, Number(senderId)),
    ...
    const chat = await db.query.chats.findFirst({
      where: eq(chats.id, conversationId),
      with: {
    ...
    const combinedBody = unreadMessages.map((m: any) => {
...
router.get('/conversations', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const userConvs = await db.query.chats.findMany({
      where: or(eq(chats.user1Id, uid), eq(chats.user2Id, uid)),
      with: {
...
router.post('/init', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    let conversation = await db.query.chats.findFirst({
      where: and(eq(chats.user1Id, user1Id), eq(chats.user2Id, user2Id), adCondition),
    });
...
router.get('/history', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    const conversation = await db.query.chats.findFirst({
      where: eq(chats.id, conversationId as string),
      with: {
        user1: { columns: { id: true, username: true, fullName: true, profileImg: true, isDeactivated: true } },
        user2: { columns: { id: true, username: true, fullName: true, profileImg: true, isDeactivated: true } },
        ad: {
    ...
    if (cursor) {
      ...
      if (!isNaN(cursorNum)) {
        conditions.push(lt(messages.id, cursorNum));
      }
    }
  ...
  try {
    ...
    if (numericIds.length > 0) {
      ...
      await db.update(messages)
        .set({ readAt: now })
        .where(inArray(messages.id, numericIds));
...
router.post('/like', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    if (isNaN(msgIdNum)) return res.status(400).json({ success: false, error: 'Invalid Message ID' });

    const msg = await db.query.messages.findFirst({ where: eq(messages.id, msgIdNum) });
    if (!msg) {
...
router.patch('/edit', requireAuth, async (req: Request, res: Response) => {
  try {
...
router.delete('/delete/:id', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    await db.delete(messages).where(eq(messages.id, msgIdNum));
