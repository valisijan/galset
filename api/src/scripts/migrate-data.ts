  ...
  try {
    ...
    for (const row of countriesRes.rows) {
      await supabase.query(
        'INSERT INTO "Country" (id, name, slug) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING',
    ...
    for (const row of citiesRes.rows) {
      await supabase.query(
        'INSERT INTO "City" (id, name, slug, "countryId", lat, lng) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO NOTHING',
        [row.id, row.name, row.slug, row.countryId, row.lat, row.lng]
      );
    }
    ...
    for (const row of categoriesRes.rows) {
      await supabase.query(
        'INSERT INTO "Category" (id, name, slug, icon, "parentId") VALUES ($1, $2, $3, $4, $5) ON CONFLICT (id) DO NOTHING',
        [row.id, row.name, row.slug, row.icon, row.parentId]
      );
    }
    ...
    for (const row of filtersRes.rows) {
      await supabase.query(
        'INSERT INTO "Filter" (id, name, slug, type, options, source) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (id) DO NOTHING',
        [row.id, row.name, row.slug, row.type, JSON.stringify(row.options), row.source]
      );
    }
    ...
    for (const row of catFiltersRes.rows) {
      await supabase.query(
        'INSERT INTO "FilterUse" ("categoryId", "filterId") VALUES ($1, $2) ON CONFLICT DO NOTHING',
        [row.categoryId, row.filterId]
      );
    }
    ...
    for (const row of brandModelsRes.rows) {
      await supabase.query(
        'INSERT INTO "Brand" (id, type, brand, models) VALUES ($1, $2, $3, $4) ON CONFLICT (id) DO NOTHING',
        [row.id, row.type, row.brand, JSON.stringify(row.models)]
      );
    }
    ...
    for (const row of pricingRes.rows) {
      await supabase.query(
        'INSERT INTO "Pricing" (id, category, name, price, currency, features, "createdAt", "updatedAt") VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (id) DO NOTHING',
        [row.id, row.category, row.name, row.price, row.currency, JSON.stringify(row.features), row.createdAt, row.updatedAt]
      );
    }
