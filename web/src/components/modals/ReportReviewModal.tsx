...
export default function ReportReviewModal({ isOpen, onClose, reviewId, onBack }: ReportReviewModalProps) {
    ...
    useEffect(() => {
        ...
        window.history.pushState({ modal: "reportReview" }, "");
        ...
        return () => {
            ...
            if (window.history.state?.modal === "reportReview") {
                window.history.back();
            }
        };
    }, [isOpen, onBack]);
    ...
    const reviewReasonOptions = [
        "Osvetnička ocena",
        "Korišćenje uvreda i govora mržnje",
        "Otkrivanje ličnih podataka",
        "Ocena sadrži laži",
        "Greška korisnika",
        "Spoljašnji faktori",
        "Reklamiranje u oceni",
        "Drugo",
    ];
        ...
        return (
            <>
                ...
                <div className="pt-8 pb-3 text-center">
                    <h3 className="text-text-main font-bold text-lg">Prijavi ocenu</h3>
                </div>
                ...
                <div className="w-full px-6 pb-6 flex flex-col gap-3">
                    ...
                    {onBack && (
                        <button
                            onClick={onBack}
                            className="w-full py-3 rounded-full bg-bg-3 hover:bg-bg-3/80 text-white font-bold transition-all duration-200 cursor-pointer text-base"
                        >
                            Nazad
                        </button>
                    )}
