...
import { db } from "@/db";
import { wishlists, ads as adsTable } from "@/db/schema";
...
export const metadata: Metadata = {
    title: "Lista želja - Galset",
};

function WishlistSkeleton({ viewMode = 'grid' }: { viewMode?: 'grid' | 'list' }) {
    return (
...
async function WishlistData({ searchParams, viewMode }: { searchParams: any, viewMode: 'grid' | 'list' }) {
    ...
    const whereClause: any = search ? and(eq(wishlists.userId, user.id), adCondition) : eq(wishlists.userId, user.id);

    const [wishlistItems, totalResult] = await Promise.all([
        db.query.wishlists.findMany({
            where: whereClause,
            limit: adsPerPage,
            with: {
                ad: {
    ...
    const fetchedAds = wishlistItems.map((item: any) => item.ad);
    const serializedAds = JSON.parse(JSON.stringify(fetchedAds));

    return <WishlistClient initialAds={serializedAds} initialTotal={total} initialViewMode={viewMode} />;
}

export default async function WishlistPage(props: { searchParams: Promise<any> }) {
    const searchParams = await props.searchParams;
    const cookieStore = await cookies();
    const viewMode = (cookieStore.get("galset_view_wishlist")?.value || "grid") as 'grid' | 'list';

    return (
        <div className="min-h-screen bg-bg-1 text-text-main">
            <Suspense fallback={<WishlistSkeleton viewMode={viewMode} />}>
                <WishlistData searchParams={searchParams} viewMode={viewMode} />
            </Suspense>
        </div>
    );
}
